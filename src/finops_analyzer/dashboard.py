...
def create_analysis_table(analyses: dict[str, StockAnalysis]) -> Table:
    ...
    for symbol, analysis in analyses.items():
        table.add_row(
            symbol,
            Text(format_percent(analysis.price_change_1d), style=get_percent_style(analysis.price_change_1d)),
            Text(format_percent(analysis.price_change_7d), style=get_percent_style(analysis.price_change_7d)),
            Text(format_percent(analysis.price_change_30d), style=get_percent_style(analysis.price_change_30d)),
...
def create_insights_panel(analysis: PortfolioAnalysis) -> Panel:
    ...
    if analysis.diversification_score is not None:
        ...
        score_style = "green" if analysis.diversification_score >= 70 else "yellow" if analysis.diversification_score >= 40 else "red"
...
def create_news_panel(analyses: dict[str, StockAnalysis]) -> Panel:
    ...
    for symbol, analysis in analyses.items():
        ...
        for article in analysis.news_articles[:3]:
            ...
            content.append(f"{article.title[:60]}...\n" if len(article.title) > 60 else f"{article.title}\n", style="")
            content.append(f"     {article.source} • {article.published_at.strftime('%Y-%m-%d')}\n", style="dim")
