...
class Portfolio(BaseModel):
    ...
    def total_value(self) -> Decimal:
        ...
        return sum(
            (h.current_value or Decimal("0") for h in self.holdings),
            Decimal("0")
        )
    ...
    def get_allocation(self) -> dict[str, float]:
        ...
        if self.total_value == 0:
            return {}
        return {
            h.symbol: float((h.current_value or Decimal("0")) / self.total_value * 100)
    ...
    def get_sector_allocation(self) -> dict[str, float]:
        ...
        return {
            sector: float(value / self.total_value * 100)
