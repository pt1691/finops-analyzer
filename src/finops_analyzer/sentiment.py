...
class AIAnalyzer:
    ...
    def generate_portfolio_insights(
        ...
        for symbol, analysis in stock_analyses.items():
            analyses_text += f"\n{symbol}:\n"
            analyses_text += f"  - 30-day change: {analysis.price_change_30d:.1f}%\n" if analysis.price_change_30d else ""
            ...
            analyses_text += f"  - Sentiment: {analysis.overall_sentiment.value if analysis.overall_sentiment else 'Unknown'}\n"
            if analysis.risk_factors:
                analyses_text += f"  - Risk factors: {', '.join(analysis.risk_factors)}\n"
        
        user_prompt = PORTFOLIO_INSIGHTS_PROMPT.format(
            portfolio_summary=portfolio_summary,
            stock_analyses=analyses_text
        )
        
        response = self._call_ai(
            "You are a senior portfolio manager. Respond only with valid JSON.",
            user_prompt
        )
