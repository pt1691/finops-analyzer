...
from .dashboard import console, display_full_report
...
def analyze(
    ...
    symbols: str | None = typer.Option(
        None, "--symbols", "-s", help="Comma-separated stock symbols (e.g., AAPL,GOOGL,MSFT)"
    ),
    no_news: bool = typer.Option(False, "--no-news", help="Skip news and sentiment analysis"),
    no_ai: bool = typer.Option(False, "--no-ai", help="Skip AI-powered insights (runs faster, no API costs)"),
    output_json: Path | None = typer.Option(None, "--output", "-o", help="Save analysis results to JSON file"),
):
    """
    ...
    # Run analysis
    analysis = analyzer.analyze_with_progress(portfolio, include_news=not no_news, include_ai_insights=include_ai)
...
@app.command()
def quote(symbol: str = typer.Argument(..., help="Stock symbol to look up")):
    """
...
def demo():
    ...
    analyzer = get_analyzer()
    analysis = analyzer.analyze_with_progress(portfolio, include_news=True, include_ai_insights=False)
