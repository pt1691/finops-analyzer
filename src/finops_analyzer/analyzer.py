...
from .sentiment import get_news_fetcher, get_ai_analyzer
from .dashboard import create_progress_bar, console
        ...
        if include_news:
            for holding in portfolio.holdings:
                ...
                if articles and self.settings.get_active_api_key():
                    articles, overall_sentiment, summary = self.ai_analyzer.analyze_sentiment(
                        holding.symbol, 
                        articles
                    )
...
Sector Allocation: {portfolio.get_sector_allocation()}
            ...
            insights = self.ai_analyzer.generate_portfolio_insights(
                portfolio_summary,
